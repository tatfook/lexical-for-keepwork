...
function forkModuleContent(
  ...
  if (target === 'cjs') {
    lines.push(
      `'use strict'`,
      `const ${outputFileName} = ['development', 'test'].includes(process.env.NODE_ENV) ? require('${devFileName}') : require('${prodFileName}');`,
    ...
    if (target === 'esm') {
      ...
      lines.push(
        `const mod = await (['development', 'test'].includes(process.env.NODE_ENV) ? import('${devFileName}') : import('${prodFileName}'));`,
      );
    }
