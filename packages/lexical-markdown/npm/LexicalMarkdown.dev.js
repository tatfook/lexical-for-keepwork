...
function exportChildren(node, elementTransformers, textTransformersIndex, textMatchTransformers) {
  ...
  mainLoop: for (const child of children) {
    ...
    if (lexical.$isLineBreakNode(child)) {
      ...
      output.push(exportChildren(child, elementTransformers, textTransformersIndex, textMatchTransformers)); // don't need a line break after the last child
...
function createMarkdownImport(transformers) {
  ...
  return (markdownString, node) => {
    ...
    root.clear();
    parseMarkdownString(root, lines, byType); // Removing empty paragraphs as md does not really
