...
import { $getRoot, $isElementNode, $isDecoratorNode, $isLineBreakNode, $isTextNode, $getSelection, $createParagraphNode, $createTextNode, $isParagraphNode, $createLineBreakNode, $isRangeSelection, $isRootOrShadowRoot, $createRangeSelection, $setSelection } from 'lexical';
import { $isListNode, $isListItemNode, ListNode, ListItemNode, $createListItemNode, $createListNode } from '@lexical/list';
import { $isQuoteNode, HeadingNode, $isHeadingNode, $createHeadingNode, QuoteNode, $createQuoteNode } from '@lexical/rich-text';
      ...
      if (elementTransformer.getNumberOfLines) {
        ...
        if (match) {
          if (elementTransformer.recursivelyParse) {
            ...
            const textNode = $createTextNode(lines.slice(i + 1, i + numberOfLines).join('\n'));
            const elementNode = $createParagraphNode();
           ...
           */
          if (i < linesLength && elementTransformer.closeRegExp && lines[i].match(elementTransformer.closeRegExp)) {
            i++;
          }
          isMatched = true;
          break;
...
export { $convertFromMarkdownString, $convertToMarkdownString, BOLD_ITALIC_STAR, BOLD_ITALIC_UNDERSCORE, BOLD_STAR, BOLD_UNDERSCORE, CHECK_LIST, CODE, ELEMENT_TRANSFORMERS, HEADING, HIGHLIGHT, INLINE_CODE, ITALIC_STAR, ITALIC_UNDERSCORE, LINK, ORDERED_LIST, QUOTE, STRIKETHROUGH, TEXT_FORMAT_TRANSFORMERS, TEXT_MATCH_TRANSFORMERS, TRANSFORMERS, UNORDERED_LIST, createBlockNode, registerMarkdownShortcuts };
